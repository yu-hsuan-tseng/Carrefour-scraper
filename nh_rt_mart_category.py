        ...
        for t in tag:
            try:
                ...
                if t.find_all("a",href=True)[0]['href'][:4] != "http":
                    l1_link.append(url+t.find_all("a",href=True)[0]['href'])
                else:
...
def main():
    ...
    rt_mart_url = "https://www.rt-mart.com.tw/nh2/"
    l1,l1_link = get_l1(rt_mart_url)
    
    df = get_category(l1,l1_link)
    
    
    df.to_excel(dt+"_nh_RT_Mart_Category.xlsx",index=False,engine='xlsxwriter')
    df.to_csv(dt+"_nh_RT_Mart_Category.csv",index=False)
